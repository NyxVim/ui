...
return function()
  ...
  -- list all hl groups that start with NyxVimHead
  -- thanks to @max397574 for teaching me
  local highlights_raw = vim.split(api.nvim_exec("filter " .. "NyxVimHead" .. " hi", true), "\n")
  local highlight_groups = {}

  for _, raw_hi in ipairs(highlights_raw) do
    table.insert(highlight_groups, string.match(raw_hi, "NyxVimHead" .. "%S+"))
  end
  ...
  for i = 0, #ascii_header - 1, 1 do
    api.nvim_buf_add_highlight(buf, nyxcheatsheet, "NyxVimAsciiHeader", i, 0, -1)
  end
